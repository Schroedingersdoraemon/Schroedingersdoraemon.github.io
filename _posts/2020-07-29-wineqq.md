---
layout: post
title:  "如何在Linux下完美wine出QQ和微信?"
date:   2020-07-29 19:23:56 +0800
categories: jekyll update
---
不少人可能遇到过这样的问题... 跟着网上教程安装wine之后，打开的程序总有千奇百怪的问题。甚至安装包的中文字体变成了方框。
如果不想使用deepin.wine.qq.im（依赖爆炸）那么如何提升QQ和微信的wine体验呢？
## 一、 安装
安装wine，下载QQ和微信的安装程序。
...
locale -a
```
如果没有zh_CN开头的输出，那么请编辑/etc/locale-gen，取消zh_CN.UTF-8前的注释
...
...
```
值得注意的是：现有字体应该为/usr/share/fonts/文件夹下的字体**文件名**

这里提供一下我的替换文件，仅供参考：<a target="_blank" href="https://github.com/Schroedingersdoraemon/dylan/blob/master/chn_font.reg">fonts.reg</a>
...
我替换的现有字体为*nerd fonts source code pro*。Arch用户可在archlinuxcn源中下载nerd-fonts-source-code-pro。其他用户可在nerd fonts的<a target="_blank" href="https://github.com/ryanoasis/nerd-fonts">GitHub主页</a>自行下载安装
...
#### (2). 下载[WeChatWin.dll](https://github.com/Schroedingersdoraemon/dylan/blob/master/prog/WeChatWin.dll.zst)文件，通过如下命令解压：
```bash
zstd -d WeChatWin.dll.zst
```
#### 将解压后文件放在~/.wine/drive_c/Tencent/WeChat/文件夹下（你的微信的安装路径），此时微信也可以正常打开了。
